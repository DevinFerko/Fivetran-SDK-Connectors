    ...
    def read_stream(self, connector: Connector) -> Iterator[Dict[str, Any]]:
        ...
        while True:
            ...
            for chat in chats:
                ...
                # Check for the latest timestamp for state tracking
                # The LiveChat API returns a list of objects sorted by start time descending,
                # so the first record might be the newest. Always compare against current new_last_synced_time.
                chat_start_time = datetime.fromisoformat(chat["started_at"]).replace(tzinfo=timezone.utc)
            ...
            # Handle Pagination: Check if more pages are available
            # LiveChat API uses `next_page_id` or similar for cursor-based pagination
            # For this simple example, we increment a page number.
            # You must adapt this to the actual LiveChat pagination model.
            
            # --- Hypothetical Paging Logic (Replace with actual LiveChat mechanism) ---
            if len(chats) < params["limit"]:
                log.info("End of results for the current date range.")
                break
            
            params["page"] += 1


        # Update the state with the new high-water mark timestamp
        state[self.TABLE_NAME] = {
            "last_synced_time": new_last_synced_time.isoformat().replace('+00:00', 'Z')
        }
        
        # The stream MUST yield the final state before closing
...
class LiveChatConnector(BaseConnector):
    """
    The main Fivetran connector class. It defines the streams (tables) to sync.
    """
    def __init__(self):
        # Instantiate all streams (tables) that this connector will sync
        self.streams = [LiveChatChatsStream()]

    def read(self, connector: Connector) -> ReadResult:
        """
        The main method called by the Fivetran core service to initiate a sync.
        """
        log.info(f"Starting sync for LiveChat connector (Schema: {connector.schema.get('name')})")

        # 1. Yield the connector schema definition
        yield ReadResult(schema=[stream.get_schema() for stream in self.streams])

        # 2. Yield all records for each stream
        for stream in self.streams:
            for record in stream.read_stream(connector):
                if "__fivetran_state" in record:
                    # Yield the state update for the stream
                    yield ReadResult(state=record["__fivetran_state"])
                else:
                    # Yield a normal data record
                    yield ReadResult(table_name=stream.TABLE_NAME, data=record)
        
        # 3. Yield the final state once all streams are complete
        log.info("LiveChat sync complete.")
        # Fivetran SDK handles the final state compilation, but explicitly
        # yielding the stream results covers this.
